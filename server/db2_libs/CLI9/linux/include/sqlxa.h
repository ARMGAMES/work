...
#define SQLXA_MAXGTRIDSIZE  64     /* maximum size in bytes of gtrid */
#define SQLXA_MAXBQUALSIZE  64     /* maximum size in bytes of bqual */
...
#define SQLXA_XID_IS_DB2_COORDINATED( x ) ( (x).formatID   == SQLXA_XID_FORMATID )
...
#define SQLXA_XID_SQL_V7(xid) ( SQLXA_XID_IS_DB2_COORDINATED(xid) && ((xid)->bqual_length == 0))
...
//SQL_USERID_SZ is deprecated, change to 30 for discontinued functionality
#define SQLXA_USERID_SZ         30
...
int SQL_API_FN db2XaGetRmid(char *pDbAlias);
int SQL_API_FN db2XaSetTimeout(int rmid, int timeout);
