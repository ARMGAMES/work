...
#include "sqlsystm.h"
#include <stddef.h>
#include "osslib.h"          // OSSE_SHARED_LIB_NAME
#include "osslatch.h"        // OSSLatch
#include "sqlqg_wrapper.h"   // for wrapper *
#include "sqlodari.h"        // sqloModuleHandle

typedef Wrapper* (*Hook_Function_Type)(void);

// Static initializer for the latch
class FedWrapperLibLatch: public OSSLatch {
public:

   FedWrapperLibLatch(void) {init ();};
};

struct wrapperLibLatch {
    char               wrapperLibName[SQL_MAX_VAR_NAME+1];
    sqloModuleHandle   *wrapperLibHandle;
    Hook_Function_Type hook;
#ifdef DB2_DEBUG
    Sint loadcount;
#endif
};

// OSSE_SHARED_LIB_NAME(db2drda) is libdb2drda.so on Linux
// comments for two list 
static wrapperLibLatch wrapperFLibList[] =
{
   {OSSE_SHARED_LIB_NAME(db2net8),     NULL, NULL},
   {OSSE_SHARED_LIB_NAME(db2drda),     NULL, NULL},
   {OSSE_SHARED_LIB_NAME(db2informix), NULL, NULL},
   {OSSE_SHARED_LIB_NAME(db2ctlib),    NULL, NULL},
   {OSSE_SHARED_LIB_NAME(db2mssql3),   NULL, NULL},
   {OSSE_SHARED_LIB_NAME(db2teradata), NULL, NULL},
   {OSSE_SHARED_LIB_NAME(db2rcodbc),   NULL, NULL},
   {OSSE_SHARED_LIB_NAME(db2rcjdbc),   NULL, NULL}
};

static wrapperLibLatch wrapperULibList[] =
{
   {OSSE_SHARED_LIB_NAME(db2net8),     NULL, NULL},
   {OSSE_SHARED_LIB_NAME(db2drda),     NULL, NULL},
   {OSSE_SHARED_LIB_NAME(db2informix), NULL, NULL},
...
#define FENCED_WRAPPER_LIB_NUM SQLZ_ELEMENTS_IN_ARRAY(wrapperFLibList)
#define UNFENCED_WRAPPER_LIB_NUM SQLZ_ELEMENTS_IN_ARRAY(wrapperULibList)
...
public:
  ...
  //----------------------------------------------------------------
  // Constants for XML implicit parsing option
  //----------------------------------------------------------------
  enum Implicit_Xml_Parse_Option
  {
    STRIP_WHITESPACE,
    PRESERVE_WHITESPACE
  };

  // get implicit Xml parsing option
  static Implicit_Xml_Parse_Option get_implicit_xml_parse_option(void);    
};
