...
** due to differences in codepage and endianess between the client and the
** server.
** 
** Authorization
** 
...
#define DB2LOAD_MAX_HOSTNAME_SZ        255       /* Max size of               */
                                                 /* db2PartLoadIn -           */
                                                 /* iHostName                 */
#define DB2LOAD_MAX_FILETRANSFERCMD_SZ 260       /* Max size of               */
                                                 /* db2PartLoadIn -           */
                                                 /* iFileTransferCmd          */
#define DB2LOAD_MAX_USEREXITCMD_SZ     260       /* Max size of               */
                                                 /* db2LoadUserExit -         */
                                                 /* iSourceUserExitCmd        */
...
/* db2HistoryOpen input struct for DS Servers                                 */
/******************************************************************************
** db2HistoryOpenStruct980 data structure
** db2HistoryOpenStruct980 data structure parameters
** 
** piDatabaseAlias
** Input. A pointer to a string containing the database alias.
** 
...
** 
*******************************************************************************/
typedef SQL_STRUCTURE db2HistoryOpenStruct980
{
       char                  *piDatabaseAlias; /* DB to fetch history from    */
       char                  *piTimestamp; /* Since this timestamp            */
       char                  *piObjectName; /* Entries containing this        */
                                       /* object                              */
       db2Uint32             oNumRows; /* # of entries matching search        */
       db2Uint32             oMaxTbspaces; /* Max. # of tablespace names      */
                                       /* stored with ANY history entry       */
       db2Uint32             oMaxLogStreams; /* Max. # of ranges of log       */
                                       /* streams stored with ANY history     */
                                       /* entry                               */
       db2Uint16             iCallerAction; /* Caller action                  */
       db2Uint16             oHandle;  /* Handle for this scan                */
} db2HistoryOpenStruct980;
...
typedef SQL_STRUCTURE db2gHistoryOpenStruct
{
       ...
       char                  *piObjectName; /* Entries containing this        */
                                       /* object                              */
       db2Uint32             iAliasLen; /* Length of database alias string    */
       db2Uint32             iTimestampLen; /* Length of timestamp string     */
       db2Uint32             iObjectNameLen; /* Length of object name string  */
       db2Uint32             oNumRows; /* # of entries matching search        */
       db2Uint32             oMaxTbspaces; /* Max. # of tablespace names      */
                                       /* stored with ANY history entry       */
       db2Uint16             iCallerAction; /* Caller action                  */
...
typedef SQL_STRUCTURE db2HistoryEID
{
       ...
       db2Uint32             ioHID;    /* Local history file entry ID         */
} db2HistoryEID;
...
#define DB2PRODUCT_ID_SZ               DB2HISTORY_COMMENT_SZ
...
{
       char                  ioHistDataID[8]; /* Eye catcher = 'SQLUHINF'     */
       db2Char               oObjectPart; /* Start time + sequence number     */
       db2Char               oEndTime; /* Completion time of event            */
       db2Char               oFirstLog; /* First log file                     */
       db2Char               oLastLog; /* Last log file                       */
       db2Char               oID;      /* Backup ID, or unique table ID       */
       db2Char               oTableQualifier; /* Table qualifier              */
       db2Char               oTableName; /* Table name                        */
       db2Char               oLocation; /* Location of referrenced object     */
       ...
       struct sqlca          *poEventSQLCA; /* Resulting SQLCA of event       */
       struct db2Char        *poTablespace; /* List of tablespace names       */
       db2Uint32             iNumTablespaces; /* I: # of available            */
                                       /* poTablespace fields                 */
       db2Uint32             oNumTablespaces; /* O: # of used poTablespace    */
                                       /* fields                              */
       char                  oOperation; /* Event type                        */
       char                  oObject;  /* Operation granularity               */
       char                  oOptype;  /* Operation details                   */
       char                  oStatus;  /* Status of entry                     */
       char                  oDeviceType; /* Device type of 'location'        */
} db2HistoryData;
/******************************************************************************
...
{
       db2LogStreamIDType    oStreamID; /* Log stream identifier of a log     */
                                       /* file                                */
       db2Uint32             oFirstLog; /* Beginning Log Extent Number        */
...
{
       db2Uint32             iNumLogStreams; /* Number of Log streams         */
       db2Uint32             oNumLogStreams; /* Number of Log streams         */
       struct db2HistoryLogStreamRange *oStream; /* list of log ranges by     */
                                       /* log stream                          */
} db2HistoryLogRange;


/* Structure db2HistoryData980                                                */
/*                                                                            */
/* Description:                                                               */
/*      ioHistDataID      -- Eye catcher, must be set to "SQLUHINF"           */
/*                                                                            */
...
/******************************************************************************
** db2HistoryData980 data structure for DS
** This structure is used to return information after a call to the
** db2HistoryGetEntry980 API.
** 
** Table: Fields in the db2HistoryData980 Structure 
** ----------------------------------------------------------------------
** |Field Name      |Data Type |Description                              |
** |---------------------------------------------------------------------|
** |ioHistDataID    |char(8)   |An 8-byte structure identifier and       |
...
** |---------------------------------------------------------------------|
** |oStatus         |char      |Entry status: A for action, D for deleted|
** |                |          |(future use), E for expired, I for       |
** |                |          |inactive, N for not yet committed, Y for |
** |                |          |committed or active, a for active backup,|
** |                |          |but some datalink servers have not yet   |
** |                |          |completed the backup, and i for inactive |
** |                |          |backup, but some datalink servers have   |
** |                |          |not yet completed the backup.            |
** |---------------------------------------------------------------------|
** |oDeviceType     |char      |Device type. This field determines how   |
** |                |          |the oLocation field is interpreted: A for|
** |                |          |TSM, C for client, D for disk, K for     |
...
** 
*******************************************************************************/
typedef SQL_STRUCTURE db2HistoryData980
{
       char                  ioHistDataID[8]; /* Eye catcher = 'SQLUHINF'     */
       db2Char               oObjectPart; /* Start time + sequence number     */
       db2Char               oEndTime; /* Completion time of event            */
       ...
       db2Uint32             oNumTablespaces; /* O: # of used poTablespace    */
                                       /* fields                              */
       db2HistoryLogRange    ioLogRange; /* Log ranges 1 per stream           */
       char                  oOperation; /* Event type                        */
       char                  oObject;  /* Operation granularity               */
       char                  oOptype;  /* Operation details                   */
       char                  oStatus;  /* Status of entry                     */
...
{
       struct db2HistoryData *pioHistData; /* History entry data area         */
       db2Uint16             iHandle;  /* History scan handle                 */
       db2Uint16             iCallerAction; /* Caller action                  */
} db2HistoryGetEntryStruct;

/******************************************************************************
** db2HistoryGetEntryStruct980 data structure
** db2HistoryGetEntryStruct980 data structure parameters
** 
** pioHistData
** Input. A pointer to the db2HistData980 structure.
** 
** iHandle
** Input. Contains the handle for scan access that was returned by the
** db2HistoryOpenScan API.
...
** 
*******************************************************************************/
typedef SQL_STRUCTURE db2HistoryGetEntryStruct980
{
       struct db2HistoryData980 *pioHistData; /* History entry data area      */
       db2Uint16             iHandle;  /* History scan handle                 */
       db2Uint16             iCallerAction; /* Caller action                  */
} db2HistoryGetEntryStruct980;
...
typedef SQL_STRUCTURE db2HistoryUpdateStruct
{
       ...
       char                  *piNewComment;
       char                  *piNewStatus;
       db2HistoryEID         iEID;     /* ID of the entry to be updated       */
...
** 
** iFirstLog
** Input. The first log extent needed to rollforward the backup.
** 
** iLastLog
** Input. The last log extent needed to rollforward the backup.
** 
** iLastLSN
...
{
       char                  iProductID[DB2PRODUCT_ID_SZ+1];
...
{
       db2Uint32             *piSqlStmtLen; /* SQL statement text length      */
       char                  *piSqlStmt; /* SQL statement text                */
       db2Uint32             *piLineNum; /* Source Line Number                */
       struct sqla_flaginfo  *pioFlagInfo; /* SQL Flagger information         */
       ...
       db2Uint16             *poSqlStmtType; /* Type of SQL statement         */
       char                  *poBuffer1; /* 256 byte string buffer 1          */
       char                  *poBuffer2; /* 256 byte string buffer 2          */
       char                  *poBuffer3; /* 256 byte string buffer 3          */
...
{
       db2Uint16             *piProgramNameLength; /* Program name length     */
       char                  *piProgramName; /* Program name                  */
       db2Uint16             *piDbNameLength; /* Database name length         */
       char                  *piDbName; /* Database name                      */
       db2Uint16             *piDbPasswordLength; /* Password length          */
       char                  *piDbPassword; /* Password                       */
       db2Uint16             *piBindNameLength; /* Bindfile name length       */
       char                  *piBindName; /* Bindfile name                    */
       struct sqla_options   *piOptionsArray; /* Precompiler options array    */
       db2Uint16             *piPidLength; /* Precompiler program ID buffer   */
                                       /* length                              */
       struct sqla_program_id *poPrecompilerPid; /* Precompiler program ID    */
} db2InitStruct;
...
#define DB2REORG_RECLAIM_EXTENTS  10      /* Define for reorg reclaim         */
                                          /* extents                          */
...
** No clean up is required, used when DB2REORG_OBJ_INDEXESALL is specified as
** the reorgType . 
...
** No conversion is required, used when DB2REORG_OBJ_INDEXESALL is specified as
** the reorgType . 
** - DB2REORG_CONVERT 
** Convert to type 2 index, used when DB2REORG_OBJ_INDEXESALL is specified
** as the reorgType . 
** 
** nodeListFlag
...
** fails on any of the affected nodes, then only the failing nodes will have
** the table reorganization rolled back.
** 
** Note:
...
/* iAllNodeFlag values                                                        */
#define DB2BACKUP_NODE_LIST   DB2_NODE_LIST /* Submit to all nodes in the     */
                                            /* node list                      */
#define DB2BACKUP_ALL_NODES   DB2_ALL_NODES /* Submit to all nodes in the     */
                                            /* node configuration file        */
#define DB2BACKUP_ALL_EXCEPT  DB2_ALL_EXCEPT /* Submit to all nodes except    */
                                            /* the ones specified by the      */
                                            /* nodelist parameter             */
...
** database status and a node number. 
** 
...
** path is the default overflow log path for each node. In a single-partition
** database environment, the overflow log path can be relative if the server
** is local. 
** 
** iNumChngLgOvrflw 
** Input. Partitioned database environments only. The number of changed overflow
...
** indicating whether the operation should apply to all nodes listed in
...
** 
** iNumNodes
** Partitioned database environment only. Input. Specifies the number of nodes
** in the piNodeList array.
** 
** piNodeList
** Partitioned database environment only. Input. A pointer to an array of node
** numbers against which to apply the archive log operation.
** 
** iOptions
** Input. Reserved for future use.
** 
...
** 
** initialLSN
** The first LSN used, or that will be used, by the database since it was
** activated.
** 
** firstReadLSN
...
** this LSN as the start of the next read instead of the end of the last
** log record in poLogBuffer parameter will prevent rescanning log records
** which have already been filtered.
** 
** firstReusedLSN
** The first LSN to be reused due to a database restore or rollforward
** operation.
** 
** logRecsWritten
...
{
   db2LSN                              initialLSN; /* Initial LSN at first    */
                                                 /* connect                   */
   db2LSN                              firstReadLSN; /* First LSN read on     */
                                                 /* call                      */
   db2LSN                              nextStartLSN; /* Next LSN caller       */
                                                 /* should read               */
   db2LSN                              firstReusedLSN; /* First LSN reused    */
                                                 /* due to restore            */
                                                 /* rollforward               */
   ...
   db2Uint32                           reserved;} db2ReadLogInfoStruct;
...
** code associated with the error. For permanent, SQL0204N will be returned.
...
{
   db2LSN                              recordLSN; /* Record log sequence      */
                                                 /* number                    */
...
** byte address for the reading of the log. This value must be greater than
** the startLsn parameter, and does not need to be the end of an actual log
** record. 
** 
** poLogBuffer 
...
** of 56 bytes. Its maximum size is dependent on the size of the requested
** range.
...
** by an eight byte log sequence number (LSN), representing the LSN of the
...
** log sequence number range and a buffer to hold the log records. This
** API reads the log sequentially, bounded by the requested LSN range,
...
** as the end of the current log.
** 
** The propagatable log records read within the starting and ending LSN range
** are returned in the log buffer. A log record does not contain its LSN; it
...
** nextStartLSN field returned in the db2ReadLogStruct structure. Resubmit
** the call, with this new starting LSN and a valid ending LSN. The next
...
** 
** piNodeName 
** Input. Name of the node that owns the recovery logs being read. This is
** required if the retrieve option above is specified. 
** 
** iReadLogMemoryLimit 
...
#define db2LsnToString(_inLsn, _outStr) \
         sprintf(_outStr, "%016I64X", (_inLsn).lsnU64)
...
** 
** iNodeNumber
** Input. The node where the request is to be sent. Based on this value, the
** request will be processed for the current node, all nodes or a user
** specified node. Valid values are:
** - SQLM_CURRENT_NODE
** - SQLM_ALL_NODES. Only allowed when the iVersion parameter is set to
** SQLM_DBMON_VERSION7 or SQLM_DBMON_VERSION8.
** - node value
** 
** Note:
...
** to the node specified by the DB2INSTANCE environment variable.
** 
** API include file
** 
** db2ApiDf.h
...
/******************************************************************************
** db2SetSyncSessionStruct data structure
** db2SetSyncSessionStruct data structure parameters
** 
** piSyncSessionID
** Input. Specifies an identifier for the synchronization session that
** a satellite will use. The specified value must match the appropriate
** application version for the satellite's group, as defined at the
...
/******************************************************************************
** db2SetSyncSession API
** Sets the synchronization session for a satellite. A synchronization
** session is associated with the version of the user application
** executing on the satellite. Each version of an application is
** supported by a particular database configuration, and
** manipulates particular data sets, each of which can be synchronized
** with a central site.
** 
** Authorization
** 
...
/******************************************************************************
** db2GetSyncSessionStruct data structure
** db2GetSyncSessionStruct data structure parameters
** 
** poSyncSessionID
** Output. Specifies an identifier for the synchronization session that a
** satellite is currently using.
** 
*******************************************************************************/
...
/******************************************************************************
** db2QuerySatelliteProgressStruct data structure
** db2QuerySatelliteProgressStruct data structure parameters
** 
** oStep
** Output. The current step of the synchronization session (defined in
** db2ApiDf header file, located in the include directory). 
** 
** oSubstep 
** Output. If the synchronization step indicated by parameter, oStep, can
** be broken down into substeps, this will be the current substep.
** 
** oNumSubsteps 
...
** 
** oScriptStep 
** Output. If the current substep is the execution of a script, this parameter
** reports on the progress of the script execution, if available. 
** 
** oNumScriptSteps 
** Output. If a script step is reported, this parameter contains the
** total number of steps that comprise the script's execution. 
** 
** poDescription 
...
/******************************************************************************
** db2SyncSatelliteTest API
** Tests the ability of a satellite to synchronize that is, tests
** whether the satellite can be brought to a state that is consistent
** with the other satellites of its group.
** 
** Authorization
** 
** None
...
/******************************************************************************
** db2SyncSatellite API
** Synchronizes a satellite. Satellite synchronization involves bringing
** a satellite to a state that is consistent with the other satellites
** of its group.
** 
** Authorization
** 
...
/******************************************************************************
** db2SyncSatelliteStop API
** Stops the satellite's currently active synchronization session. The
** session is stopped in such a way that synchronization for this
** satellite can be restarted where it left off by invoking
** db2SyncSatellite.
** 
** Authorization
** 
...
** 
** piProfile
** Input. The name of the profile file to be executed at each node to define
** the DB2 environment (MPP only). This file is executed before the nodes
** are started. The default value is db2profile. 
** 
** iIsNodeNum
** Input. Indicates whether a node number is specified. If specified, the start
** command only affects the specified node.
** 
** iNodeNum
...
** 
** iTblspaceNode
** Input. Specifies the node number from which the system temporary table
** space definitions should be obtained. The node number must exist in the
** db2nodes.cfg file, and is only used if the tblspace_type field is set to
** SQLE_TABLESPACES_LIKE_NODE.
** 
** iIsComputer
...
** partition only. In a partitioned database environment, it is the
** collection of all logical database partition servers defined in the node
...
** 
** iIsNodeNum
** Input. Indicates whether a node number is specified. Possible values are
** TRUE and FALSE. If specified, the stop command only affects the specified
** node.
** 
** iNodeNum
** Input. The database partition number.
