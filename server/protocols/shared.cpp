...
void TicketType::composeData(CommMsgBody& body) const
{
	// new members must go at the end
	body
		.composeUINT32(ticketTypeId)
		.composeUINT32(ticketTypeCode)
		.composeString(admission)
		.composeBOOL(multiTourn)
		.composeINT32(addChips)
		;
	created.compose(body);
	expDate.compose(body);
	body
		.composeINT32(expPeriod)
		.composeSrvTime(obsolete)
		.composeUINT32(adminIntId)
		.composeINT32(value)
		.composeString(currency)
		.composeUINT32(ticketTypeFlags)
		.composeINT32(fppValue)
		.composeINT64(pmValue64) // PYR-24938
		.composeUINT32(budgetCodeId) // PYR-27615
	;
	// new members must go at the end
}

void TicketType::parseData(CommMsgParser& parser)
{
	// new members must go at the end with parseEnded() condition
	UINT32 ticketTypeCode_;
	parser
		.parseUINT32(ticketTypeId)
		.parseUINT32(ticketTypeCode_)
		.parseStringP(admission)
		.parseBOOL(multiTourn)
		.parseINT32(addChips)
		;
	created.parse(parser);
	expDate.parse(parser);
	parser
		.parseINT32(expPeriod)
		.parseSrvTime(obsolete)
		.parseUINT32(adminIntId)
		.parseINT32(value)
		.parseStringP(currency)
		.parseUINT32(ticketTypeFlags)
		.parseINT32(fppValue)
		.parseINT64(pmValue64) // PYR-24938
		.parseUINT32(budgetCodeId); // PYR-27615
	// new members must go at the end with parseEnded() condition
	
	ticketTypeCode = (TicketTypeCode)ticketTypeCode_;
}

void TicketType::logTicketType() const
{
	PLog( "TicketTypeId %d, ticketTypeFlags %d, ticketTypeCode %d, obsolete %s, currency '%s', admission '%s', budgetCode %u", ticketTypeId, ticketTypeFlags, ticketTypeCode,
		(obsolete.isNull() ? "No" : "Yes"), currency.c_str(), admission.c_str(), budgetCodeId );
}

void TicketType::getTicketExpiration(const SrvTime& srvNow, SrvTime& ticketExpiration) const
{
	if(!expDate.isNull())
	{
		// expDate date overrules expPeriod, if both defined (which should not happen)
		ticketExpiration = expDate;
	}
	else if(expPeriod)
	{
		// Calculate ticket expiration data based on ticket type's expiration period
		time_t expTime = srvNow.encode();
		expTime += SECONDS_IN_A_DAY * expPeriod;
		ticketExpiration.decode(expTime);
	}
	else
	{
		// Neither expDate nor expPeriod
		ticketExpiration.setNull();
	}
}

bool TicketType::isChangeAllowed(const TicketType& changedTicketType) const
{
	return !(isFinalizedTicket() && (
		changedTicketType.ticketTypeFlags != ticketTypeFlags ||
		changedTicketType.ticketTypeCode != ticketTypeCode ||
		!changedTicketType.admission.equals(admission) ||
		changedTicketType.multiTourn != multiTourn ||
		changedTicketType.addChips != addChips ||
		!changedTicketType.expDate.equals(expDate) ||
		changedTicketType.expPeriod != expPeriod ||
		changedTicketType.value != value ||
		changedTicketType.fppValue != fppValue ||
		changedTicketType.pmValue64 != pmValue64 ||
		changedTicketType.budgetCodeId != budgetCodeId));
}
...
{
	switch( accountLicenseId )
	{
//	<<== @license2-domain-old: Domain name by license (obsolete?) @ ====
	case eLicenceItaly:
		return DOMAIN_NAME_IT;
	case eLicenceFrance:
		return DOMAIN_NAME_FR;
	case eLicenceEstonia:
		return DOMAIN_NAME_EE;
	case eLicenceBelgium:
		return DOMAIN_NAME_BE;
	case eLicenceSpain:
		return DOMAIN_NAME_ES;
	case eLicenceDenmark:
		return DOMAIN_NAME_DK;
	case eLicenceEU:
		return DOMAIN_NAME_EU;
	case eLicenceFB:
		return DOMAIN_NAME_NET;
	case eLicenceUK:
		return DOMAIN_NAME_UK;
	case eLicenceBG:
		return DOMAIN_NAME_BG;
	case eLicenceDESH:
		return DOMAIN_NAME_DESH;
	case eLicenceRomania:
		return DOMAIN_NAME_ROM;
	case eLicencePortugal:
		return DOMAIN_NAME_PT;
	case eLicenceCZ:
		return DOMAIN_NAME_CZ;
	case eLicenceIndia:
		return DOMAIN_NAME_IN;
	case eLicenceRU:
		return DOMAIN_NAME_RU;
	case eLicenceSE:
		return DOMAIN_NAME_SE;
	case eLicenceCH:
		return DOMAIN_NAME_CH;
	case eLicenceDE:
		return DOMAIN_NAME_DE;
	case eLicenceGR:
		return DOMAIN_NAME_GR;
	default:
		return DOMAIN_NAME_COM;
//	==== @license2-domain-old: Domain name by license (obsolete?) @ ==>>
	}
}


// TODO: move to licenserules.h
// PYR-27757
const char* getFullDomainByLicense( UINT32 accountLicenseId )
{
	switch( accountLicenseId )
	{
//	<<== @license2-domain: Domain name by license @ ====
	case eLicenceItaly:
		return "pokerstars.it";
	case eLicenceFrance:
		return "pokerstars.fr";
	case eLicenceEstonia:
		return "pokerstars.ee";
	case eLicenceBelgium:
		return "pokerstars.be";
	case eLicenceSpain:
		return "pokerstars.es";
	case eLicenceDenmark:
		return "pokerstars.dk";
	case eLicenceEU:
		return "pokerstars.eu";
	case eLicenceFB:
		return "pokerstars.net";
	case eLicenceUSNJ:
		return "pokerstarsnj.com";		// PYR-41679
	case eLicenceUK:
		return "pokerstars.uk";		// PYR-42909, PYR-43389
	case eLicenceBG:
		return "pokerstars.bg";
	case eLicenceDESH:
		return "sh.pokerstars.de";		// PYR-33564
	case eLicenceRomania:
		return "pokerstars.ro";
	case eLicencePortugal:
		return "pokerstars.pt";
	case eLicenceCZ:
		return "pokerstars.cz";
	case eLicenceIndia:
		return "pokerstars.in";
	case eLicenceRU:
		return "pokerstars.ru";
	case eLicenceUSPA:
		return "pokerstarsmtairycasino.com";
	case eLicenceSE:
		return "pokerstars.se";
	case eLicenceCH:
		return "pokerstars.ch";
	case eLicenceARBA:
		return "pokerstars.ar";
	case eLicenceUSCO:
		return "foxbet.com";
	case eLicenceDE:
		return "pokerstars.de";
	case eLicenceGR:
		return "pokerstars.gr";
	default:
...
const char* getBrandNameByLicense( UINT32 accountLicenseId )
{
	switch( accountLicenseId )
	{
//	<<== @brand-domains-by-license: 'PokerStars' brand domains by license @ ====
	case eLicenceItaly:
		return "PokerStars.it";
	case eLicenceFrance:
		return "PokerStars.fr";
	...
	case eLicenceFB:
		return "PokerStars.net";
	case eLicenceUSNJ:	// PYR-27443
		return "PokerStars.com";
	case eLicenceUK:	// PYR-42909, PYR-43389
		return "PokerStars.uk";
	case eLicenceBG:
		return "PokerStars.bg";
	case eLicenceDESH:	// PYR-33564
		return "PokerStars SH";
	case eLicenceRomania:
		return "PokerStars.ro";
	case eLicencePortugal:	// PYR-38665
		return "PokerStars.pt";
	case eLicenceCZ:
		return "PokerStars.cz";
	case eLicenceIndia:
...
//	<<== @license2-tld-site: TLD by site @ ====
	...
	case PokerStars_RUSOPM:
		return DOMAIN_NAME_NET;
	case PokerStars_US_NJ:	// PYR-27443
		return LICENCE_NAME_USNJ;
	case PokerStars_UK:	// PYR-42909, PYR-43389
		return DOMAIN_NAME_UK;
	case PokerStars_Bul:
		return DOMAIN_NAME_BG;
	case PokerStars_DE_SH:	// PYR-33564
		return DOMAIN_NAME_DESH;
	case PokerStars_Por:	// PYR-38665
		return DOMAIN_NAME_PT;
	case PokerStars_Rom:
		return DOMAIN_NAME_ROM;
	case PokerStars_Cze:
	...
	case PokerStars_ARBA:
		return LICENCE_NAME_ARBA;
	case PokerStars_US_CO:
		return LICENCE_NAME_USCO;
	case PokerStars_DE:
		return DOMAIN_NAME_DE;
	case PokerStars_GR:
		return DOMAIN_NAME_GR;
...
// PYR-27757
const char* getFullDomainBySite( UINT32 siteId, const char* country )
{
	switch( siteId )
	{
//	<<== @site2-domain-site: Domain name by site id @ ====
	case PokerStars_Ita:
		return "pokerstars.it";
	case PokerStars_Fra:
		return "pokerstars.fr";
	...
	case PokerStars_RUSOPM:
		return "pokerstars.net";
	case PokerStars_US_NJ:	// PYR-27443
		return "pokerstarsnj.com";
	case PokerStars_Com:
	case PokerStars_RUSO:
		if(country && *country && !PString::compareIgnoreCase(country, "ru"))
			return "pokerstars2.com"; // PYR-42022, PYR-43470
		if(country && *country && !PString::compareIgnoreCase(country, "lt")) // PYR-41495
			return "pokerstars1.eu";
		if(country && *country && !PString::compareIgnoreCase(country, "hu")) // PYR-41686, PYR-42871
			return "pokerstars5.hu.com";
		return "pokerstars.com";
	case PokerStars_UK:	// PYR-42909, PYR-43389
		return "pokerstars.uk";
	case PokerStars_Bul:
...
const char* getBrandNameBySite( UINT32 siteId )
{
	switch( siteId )
	{
//	<<== @site2-brand-site: Brand name by site id @ ====
	case PokerStars_Ita:
		return "PokerStars.it";
	case PokerStars_Fra:
		return "PokerStars.fr";
...
void readUintLicenceIdSetFromString(const char* str, set<UINT32>& uintSet, const char delimeter)
{
	vector<PString> licenseNames;
	readStrVectorFromString(str, licenseNames, delimeter);
	for (const PString& licenseName : licenseNames)
	{
		LicenceId licenseId;
		if (!LicenseRules::licenseNameToId(licenseName, licenseId) || !LicenseRules::isValidLicense(licenseId))
		{
			PLog(LOG_TRACE_FAULTY_LOGIC "readUintLicenceIdSetFromString bad config, unsupported lic='%s' in {%s}", licenseName.c_str(), str);
			continue;
		}
		uintSet.insert(licenseId);
	}
}
...
void UnCashedChequeEmailReminder::parse(CommMsgParser& parser)
{
    parser
        .parseStringP(templateName)
        .parseStringP(userId)
        .parseUINT32(transId)
        .parseINT32(dayReminder)
        .parseINT32(fxAmount)
        .parseINT32(converted)
        .parseStringP(fxCurrency)
        .parseStringP(procCurrency)
        .parseStringP(xTransStr)
        .parseSrvTime(started)
        ;
}

void UnCashedChequeEmailReminder::composeForTemplate(CommMsgBody& body) const
{
    char transIdStr[64];
    char reminderStr[8];
    char fxAmountStr[64];
    char convertedStr[64];
    char startedTime[64];

    sprintf(transIdStr, "%u", transId);
    sprintf(reminderStr, "%d", dayReminder);
    sprintf(fxAmountStr, "%d.%02d", fxAmount/100, fxAmount%100);
    sprintf(convertedStr, "%d.%02d", converted/100, converted%100);
    sprintf(startedTime, "%04d-%02d-%02d %02d:%02d:%02d", started._year, started._month, started._day, started._hour, started._minute, started._second);
        
    body
        .composeUINT32(eEmailByTemplateName)
        .composeString(templateName)
        .composeString(userId)
        .composeBYTE(eTypeEmailAddressCashier)
        .composeUINT32(7)  // 7 custom parameters below
        .composeString(startedTime)
        .composeString(fxCurrency)
        .composeString(fxAmountStr)
        .composeString(procCurrency)
        .composeString(convertedStr)
        .composeString(xTransStr)
        .composeString(transIdStr)
        ;
}

void UnCashedChequeEmailReminder::compose(CommMsgBody& body) const
{
    body
        .composeString(templateName)
        .composeString(userId)
        .composeUINT32(transId)
        .composeINT32(dayReminder)
        .composeINT32(fxAmount)
        .composeINT32(converted)
        .composeString(fxCurrency)
        .composeString(procCurrency)
        .composeString(xTransStr)
...
void translateGameType( UINT32 dbGame, BYTE& gameType , BYTE& isHiLo )
{
	...
	static const GameTranslator translators[] = {
		/*0*/{ HandTypeNone, false },	//0 none
		/*2*/{ HandTypeHoldem, false },	//1 = Holdem
		/*4*/{ HandTypeOmaha, false },	//2 = Omaha
		/*4*/{ HandTypeOmaha, true },	//3 = Omaha Hi/Lo
		/*7*/{ HandTypeStud, false },	//4 = 7 card stud
		/*7*/{ HandTypeStud, true },	//5 = stud Hi/Lo
		/*8*/{ HandTypeRazz, false },	//6 = Razz (Bug: #3872)
		/*5*/{ HandTypeDraw, false },	//7 = Draw (Bug: #3097)
		/*9*/{ HandType3Draw27, false },//8 = 3Draw(Bug: #3871)
		/*1*/{ HandTypeBadugi, false },	//9 = 3Draw(Bug: #5673)
		/*6*/{ HandTypeSingleDraw27, false }, //10 = 1Draw(Bug: #8800)
		/*3*/{ HandType5Omaha, false },	//11 = 5Omaha PYR-24197:
		/*3*/{ HandType5Omaha, true },	//12 = 5Omaha Hi/Lo
		/*11*/{ HandTypeCourchevel, false }, //13 = Courchevel Hi
		/*11*/{ HandTypeCourchevel, true },	//14 = Courchevel original (Hi/Lo)
		/*12*/{ HandType6Omaha, false },	// 15 = 6Omaha
		/*12*/{ HandType6Omaha, true },		// 16 = 6Omaha Hi/Lo
		/*13*/{ HandTypeSplitHoldem, false },	// 17 = SplitHoldem
		/*14*/{ HandTypeSplitOmaha, false },	// 18 = SplitOmaha
		/*15*/{ HandTypeShowtimeHoldem, false },  // 19 = ShowtimeHoldem
		/*16*/{ HandTypeShowtimeOmaha, false },   // 20 = ShowtimeOmaha
		/*17*/{ HandTypeFusion, false }, // 21 = Fusion
		/*18*/{ HandTypeUnfoldClassic, false }, // 22  = UnfoldClassic
		/*20*/{ HandTypeSixPlusHoldem, false }, // 23 = SixPlusHoldem
		/*21*/{ HandTypeDeepWaterHoldem, false }, // 24 = DeepWaterHoldem
		/*22*/{ HandTypeDeepWaterOmaha, false }, // 25 = DeepWaterOmaha
		/*23*/{ HandTypeSwapHoldem, false }, // 26 = SwapHoldem
		/*24*/{ HandTypeTempestHoldem, false }, // 27 = TempestHoldem
	};

	if( ( (INT32)dbGame < 0 ) || ( dbGame >= sizeof( translators )/sizeof( *translators ) ) )
		dbGame = 0;

	gameType = translators[dbGame].gameType;
	isHiLo = translators[dbGame].isHiLo;
}
...
int translateTableGame( BYTE gameType, BYTE isHiLo )
{
	...
	case HandTypeNone: // 0
		return 0; //none
	case HandTypeHoldem: // 2
		return ( 0 == isHiLo ) ? 1 : 0; // holdem / insane
	case HandTypeOmaha: // 4
		return ( 0 == isHiLo ) ? 2 : 3; // omaha / omaha HiLo
	case HandTypeStud: // 7
		return ( 0 == isHiLo ) ? 4 : 5; // stud / stud HiLo
	case HandTypeRazz: // 8  (Bug #3872:)
		return ( 0 == isHiLo ) ? 6 : 0; // razz / insane
	case HandTypeDraw: // 5  (Bug: #3097)
		return ( 0 == isHiLo ) ? 7 : 0; // draw / insane
	case HandType3Draw27: // 9  (Bug: #3871)
		return ( 0 == isHiLo ) ? 8 : 0; // 3draw / insane
	case HandTypeBadugi: // 1  (Bug: #5673)
		return ( 0 == isHiLo ) ? 9 : 0; // badugi / insane
	case HandTypeSingleDraw27: // 6  (Bug: #8800)
		return ( 0 == isHiLo ) ? 10 : 0; // 1draw / insane
	case HandType5Omaha: // 3	 PYR-24197
		return ( 0 == isHiLo ) ? 11 : 12; // 5omaha / 5omaha HiLo
	case HandTypeCourchevel: // 11	 PYR-24197
		return ( 0 == isHiLo ) ? 13 : 14; // Courchevel Hi/ Courchevel
	case HandType6Omaha: // 12
		return ( 0 == isHiLo ) ? 15 : 16; // 6omaha / 6omaha HiLo
	case HandTypeSplitHoldem: // 13  PYR-52675
		return ( 0 == isHiLo ) ? 17 : 0; // split holdem / insane
	case HandTypeSplitOmaha:  // 14
		return ( 0 == isHiLo ) ? 18 : 0; // split omaha / insane (no split omaha hilo)
	case HandTypeShowtimeHoldem: // 15
		return ( 0 == isHiLo ) ? 19 : 0; // ShowtimeHoldem / insane
	case HandTypeShowtimeOmaha:  // 16
		return ( 0 == isHiLo ) ? 20 : 0; // ShowtimeOmaha / insane (no ShowtimeOmaha hilo)
	case HandTypeFusion: // 17
		return (0 == isHiLo) ? 21 : 0; // Fusion / insane (currently no Fusion hilo)
	case HandTypeUnfoldClassic: // 18
		return (0 == isHiLo) ? 22 : 0; // UnfoldClassic / insane (currently no UnfoldClassic hilo)
	case HandTypeSixPlusHoldem: // 20
		return (0 == isHiLo) ? 23 : 0; // SixPlusHoldem / insane (SixPlusHoldem HiLo doesn't make sense)
	case HandTypeDeepWaterHoldem: // 21
		return (0 == isHiLo) ? 24 : 0; // DeepWaterHoldem / insane (DeepWaterHoldem HiLo doesn't make sense)
	case HandTypeDeepWaterOmaha: // 22
		return (0 == isHiLo) ? 25 : 0; // DeepWaterOmaha / insane (no DeepWaterOmaha HiLo)				
	case HandTypeSwapHoldem: // 23
		return (0 == isHiLo) ? 26 : 0; // SwapHoldem / insane (no Swap holdem hilo)
	case HandTypeTempestHoldem: // 24
		return (0 == isHiLo) ? 27 : 0; // TempestHoldem / insane (no Tempest holdem hilo)

	// *** IMPORTANT!!!!*** Please notify Game and Payment security team of changes and additions

	default:
		PLog("error translateTableGame ... insane? gameType=%d, isHiLo=%d", gameType, isHiLo);
		return 0;
	}
}

int translateTableMultiGame(BYTE gameType, BYTE isHiLo)
{
	switch (gameType)
	{
	case HandTypeNone: // 0
	...
	case HandTypeDeepWaterOmaha: // 22
		return (0 == isHiLo) ? 25 : 0; // DeepWaterOmaha / insane (no DeepWaterOmaha HiLo)				
	case HandTypeSwapHoldem: // 23
		return (0 == isHiLo) ? 26 : 0; // Swap Holdem / insane (no Swap Holdem HiLo)				
	case HandTypeTempestHoldem: // 24
		return (0 == isHiLo) ? 27 : 0; // TempestHoldem / insane (no Tempest holdem hilo)
	case MultiGameHose: // 100
		return (0 == isHiLo) ? 1000 : 0; // HOSE / insane 
	case MultiGameHorse: // 101
		return (0 == isHiLo) ? 1001 : 0; // HORSE / insane 
	case MultiGameEightGame: // 102
		return (0 == isHiLo) ? 1002 : 0; // Eight Game / insane 
	case MultiGameHoldemMixed: // 103
		return (0 == isHiLo) ? 1003 : 0; // Holdem mixed / insane 
	case MultiGameOmahaHiLoMixed: // 104
		return (0 == isHiLo) ? 1004 : 0; // Omaha hilo mixed / insane 
	case MultiGameHoldemOmahaPl: // 105
		return (0 == isHiLo) ? 1005 : 0; // Holdem Omaha PL / insane
	case MultiGameTripleStud: // 106
		return (0 == isHiLo) ? 1006 : 0; // Triple Stud / insane 
	case MultiGameHoldemNlOmaha: // 107
		return (0 == isHiLo) ? 1007 : 0; // Holdem NL Omaha / insane 
	case MultiGameOmahaMixed: // 108
		return (0 == isHiLo) ? 1008 : 0; // Omaha mixed / insane 
...
static const char wholeUserIntIdSign = '|';
...
bool isUserIntIdFilter(const PString& filter)
{
	...
	const char* filterString = filter.c_str();
	if (filter.length() > 1 && *filterString == wholeUserIntIdSign)
		++filterString; // ignore the special symbol

	if (::isspace((BYTE)*filterString)) // strtol first discards whitespace characters
		return false;
	char* endPtr = NULL;
	::strtol(filterString, &endPtr, 10); //-V530 //-V2547 //ok to ignore return value 
	return *endPtr == '\0';
}

// returns true if:
// 1. filter is empty
// 2. if filter is a number userIntId ends with one of the userFilter elements
// 3. if filter in a special form and if the userIntId is the same as userFilter number part
bool checkUserFilter2(UINT32 userIntId, const vector<PString>& userFilter)
{
	...
	PString strUserIntId;
	strUserIntId.appendUint( userIntId );
	for(int i = 0; i < userFilter.size(); ++i)
	{
		const PString& oneFilter = userFilter[i];
		if (oneFilter.length() > 1 && oneFilter[0] == wholeUserIntIdSign)
		{
			const char* filterString = oneFilter.c_str();
			++filterString; // ignore the special symbol
			if (strUserIntId.equals(filterString))
				return true;
			else
				continue;
		}
		if(oneFilter.length() && strUserIntId.endsWith( oneFilter ))
		{
			PTRACE5("user: %u passed filter '%s'", userIntId, oneFilter.c_str());
			return true;
		}
	}
	PTRACE5("!uf2(%u)", userIntId);
	return false;
}

// returns true if filter is empty or userId starts with one of the userFilter elements
bool checkUserFilter(const char* userId, const vector<PString>& userFilter)
{
	...
	{
		const PString& oneFilter = userFilter[i];
		if (oneFilter.length() && 0 == PString::compareIgnoreCase(userId, oneFilter, oneFilter.length()))
			return true;
	}
	return false;
}
...
void setShowSomeCardsTableFlag( UINT32& tableFlags, UINT32 lobbySetting, bool isPlayMoney, HandType handType )
{
	switch( lobbySetting )
	{
	case eShowSomeCardsOff:
		tableFlags &= ~TABLESCRIPT_FLAG_SHOW_ONE_CARD;
		break;

	case eShowSomeCardsTableSetting:
		break;

	case eShowSomeCardsPlayMoney:
		if( isPlayMoney )
		{
			tableFlags |= TABLESCRIPT_FLAG_SHOW_ONE_CARD;
		}
		break;  // for real money, based on current table flags

	case eShowSomeCardsOn:
		tableFlags |= TABLESCRIPT_FLAG_SHOW_ONE_CARD;
		break;

	default:
		PLog( "ERROR in setShowSomeCardsTableFlag, unexpected value for lobbySetting: %u", lobbySetting );
		break;
	}

	// Disable show some cards if this is a draw game or a ShowTime
	switch( handType )
	{
...
// PYR-20580 begin
void RegulatorsEventLogger::createLogFile(const char* eventLogFileName)
{
    const char* pLogFileName = PLogGetFileName();
    const char* dir = strrchr(pLogFileName, '\\');
    if (dir==NULL)
        dir = strrchr(pLogFileName, '/');
    PString filename;
    if (dir!=NULL)
        filename.assign(pLogFileName, dir+1);
    filename.
        append(eventLogFileName).
        append("log").
        appendUint(transportIndex);

    logFile = new PLogFile(filename.c_str(), 
        PLogFile::catchPErrors | 
        PLogFile::perDate |
        PLogFile::catchStdErr);

    // print format
    logFile->log("// Format (tab separated):");
    logFile->log("// EVENT: GatewayName GatewayUniqueID timestamp UserID UserIntID ");
    logFile->log("//        LicenseID LicenseProtocol InternalMessageID ExternalMessageID MessageType");
    logFile->log("//        ResponseCode ResponseStr ExtraInfo Note");
    PLog("%s created for logging %s events", filename.c_str(), eventLogFileName);
}

void RegulatorsEventLogger::writeLog(const char* messageType, 
                           const char* userId, 
                           ...
                           const char* extraInfo)
{
    ...
    const char* const emptySymbol = "''";

    SrvTime timestamp;
    timestamp.currentLocalTime();
    char dateEvt[200] = {0};
    UINT32 year = timestamp._year;
    year %= 100;
    if(!timestamp.isNull())
    {
        sprintf(dateEvt, "%02d%02d%02d%02d%02d%02d",  year, timestamp._month, 
            timestamp._day,	timestamp._hour, timestamp._minute, timestamp._second);
    }

    logFile->log("EVENT: %s\t%u\t%s\t%s\t%s\t%u\t%s\t%s\t%s\t%s\t%d\t%s\t%s\t%s", 
        loggerStationName.c_str(),      // GatewayName
        transportIndex,                 // GatewayUniqueID
        dateEvt,                        // Timestamp
        getString(userId),              // UserId
        getString(userIntId),           // UserIntID
        licenseId,                      // LicenseID
        getString(licenseProtocol),     // LicenseProtocol
        getString(internalMessageId),   // InternalMessageID
        getString(externalMessageId),   // ExternalMessageID
        messageType,                    // MessageType
        responseCode,                   // ResponseCode
        getString(responseStr),         // ResponseStr
        getString(extraInfo),           // ExtraInfo
        emptySymbol);                   // Note 
}

void RegulatorsEventLogger::checkDayChange()
{
    SrvTime timestamp;
    timestamp.currentLocalTime();
    if (logStartDay != timestamp._day)
    {
        logFile->log("// current day: %d", timestamp._day);
        logStartDay = timestamp._day;
    }
}
// PYR-20580 end
...
void MigrationUserActiveFilter::readAndPrint(const PIniFile::Section& section, const char* filterName, UINT32 gtfMask /*see MigrationGameInfo*/)
{
	const int STR_TO_MASK_SIZE = 4;
	static struct { const char* str; UINT32 mask; } strToMask[STR_TO_MASK_SIZE] =
	{
		{ "RM",  0x00 },
		{ "PM",  0x20 },
		{ "DRM", 0x40 },
	...
	PString logMsg(filterName);
	PLog("Parsing: %s", logMsg.c_str());
	logMsg.append('=');
	for (int i = 0; i < section.items.size(); ++i)
	{
		if (PString::compareIgnoreCase(section.items[i].name, filterName) != 0)
			continue;
		vector<PString> keyAndValues;
		readStrVectorFromString(section.items[i].value, keyAndValues, ':');
		if (keyAndValues.size() != 2)
		{
			PLog("Invalid %s", logMsg.c_str());
			clear();
			return;
		}
		keyAndValues[0].toUpper();
		keyAndValues[1].toUpper();
		vector<PString> fromTo;
		readStrVectorFromString(keyAndValues[0], fromTo, '.');
		if (fromTo.size() != 2)
		{
			PLog("Invalid %s", logMsg.c_str());
			clear();
		...
		LicenceId dummyLicence;
		if (!LicenseRules::licenseNameToId(fromTo[0], dummyLicence) || !LicenseRules::licenseNameToId(fromTo[1], dummyLicence))
		{
			PLog("Invalid %sFROM.TO: %s.%s", logMsg.c_str(), fromTo[0].c_str(), fromTo[1].c_str());
			clear();
			return;
		}
		logMsg.append('[').append(keyAndValues[0]).append(':');
		vector<PString> values;
		readStrVectorFromString(keyAndValues[1], values, ',');
		vector<UINT32> intValues;
		for (int j = 0; j < values.size(); ++j)
		{
			int k;
			for (k = 0; k < STR_TO_MASK_SIZE; ++k)
			{
				if (values[j].equals(strToMask[k].str))
					break;

			}
			if (k == STR_TO_MASK_SIZE)
			{
				PLog("Invalid value in %s: '%s'", logMsg.c_str(), values[j].c_str());
				clear();
				return;
			}
			intValues.push_back(strToMask[k].mask | gtfMask);
			if (intValues.size() > 1)
				logMsg.append(',');
			logMsg.appendUint(intValues[j]);
		}
		allowedTypes.insert(make_pair(keyAndValues[0], intValues));
		logMsg.append(']');
	}
	PLog("Successfully parsed: %s", logMsg.c_str());
}

bool MigrationUserActiveFilter::isPassed(UINT32 licenseFrom, UINT32 licenseTo, const vector<MigrationGameInfo>& tourns) const
{
	if (tourns.empty())
		return true;

	UINT32 toSiteId = SiteRules::getSiteIdByLicenseId(LicenceIdFlags(licenseTo));
	PString key; 
	key.append(LicenseRules::getLicenseIdCode(licenseFrom)).append('.').append(LicenseRules::getLicenseIdCode(licenseTo));
	RulesMap::const_iterator it = allowedTypes.find(key);
	if (it == allowedTypes.end()) 
	{
		PTRACE5("MigrationUserActiveFilter: no rule FROM.TO: %s", key.c_str());
		return false;
	}
	for (int i = 0; i < tourns.size(); ++i)
	{
		if ((tourns[i].sites & toSiteId) == 0)  // tourn not allowed for licenseTo
		{
			PTRACE5("MigrationUserActiveFilter: dst site 0x%X don't match mask: 0x%X", toSiteId, tourns[i].sites);
			return false;
		}
		int j;
		for (j = 0; j < it->second.size(); ++j)
		{
			if (tourns[i].type == it->second[j])  // filter match
			{
				PTRACE5("MigrationUserActiveFilter: Trn[%" PRIu64 "] passed %s:%d", tourns[i].id, key.c_str(), tourns[i].type);
				break;
			}
		}
		if (j == it->second.size())  // tried all filters, no pass
		{
			PTRACE5("MigrationUserActiveFilter: Trn[%" PRIu64 "] no pass %s:%d", tourns[i].id, key.c_str(), tourns[i].type);
			return false;
		}
	}
	// all tourns passed filters
	return true;
}

void MigrationUserActiveFilter::clear()
{
	allowedTypes.clear();
	PLog("MigrationUserActiveFilter cleared");
}
...
// MOVED from dbmanager.cpp after BUILD1026
const char* getLimitTypeComment(INT32 /*eTableLimitType*/ type, UINT32 licenseId) // PYR-112845
{
	static const char* typeComment[] = { //see enum eTableLimitType in dbm.h
		"",			// eTableLimitTypeAny - obsolete - not used (placeholder)
		"NL/PL",	// eTableLimitTypeNL
		"FL",		// eTableLimitTypeFL
		"Tourn",	// eLimitTypeTourn
		"SnG",	// eLimitTypeSNG
		"BuyIn",	// eLimitTypeBuyIn
		"Play",	// eLimitTypePlayingTime
		"BuyInDaily",	//eLimitTypeBuyInDaily
		"BuyInWeekly",	// eLimitTypeBuyInWeekly
		"BuyInMonthly",	//eLimitTypeBuyInMonthly
		"Blackjack",
		"Roulette",
		"Slots",
		"SportsStake",
		"CasinoOther",
		"SportsAccumulated", // PYR-34388 - eLimitTypeSportsAccumulated
		"", //PYR-53789 - obsolete (placeholder)
		"NetLossPerDay",
		"NetLossPerMonth",
		"SpendingLimitPerDay",
		"SpendingLimitPerMonth",
		"SportsNetLossPerDay",
		"SportsNetLossPerMonth",
		"SportsSpendingLimitPerDay",
		"SportsSpendingLimitPerMonth",
		"LoginTotalLimitPerMonth",	
		"LoginTimeWeekly",
		"LoginTimeMonthly",
		"SideBets",
		"LoginCoolOff", // PYR-105467
	};
	static_assert(SZARR(typeComment) == numLimitTypes, "typeComment needs update");

	// PYR-112845
	if (type == eLimitTypePlayingTime && (licenseId == eLicenceCZ || licenseId == eLicenceSE))
	{
		return "LoginTimeDaily";
	}

	return (0 < type && type < numLimitTypes) ? typeComment[type] : "";
}
bool isTournLimit(INT32 type)
{
	return (type == eLimitTypeTourn || type == eLimitTypeSNG);
}
bool isTableLimit(INT32 type)
{
	return (type == eTableLimitTypeNL || type == eTableLimitTypeFL);
}
bool isBuyInLimitEx(INT32 type)
{
	return (type == eLimitTypeBuyInDaily || type == eLimitTypeBuyInWeekly || type == eLimitTypeBuyInMonthly);
}
bool isCasinoLimit(INT32 type)
{
	return (type == eLimitTypeCasinoBlackjack || type == eLimitTypeCasinoRoulette || type == eLimitTypeCasinoSlots || type == eLimitTypeCasinoOther);
}
bool isSportsLimit(INT32 type)
{
	return (type == eLimitTypeSportsStake);	
}
bool isCzLimit(INT32 type)
{
	return
		type ==	eLimitTypeNetLossPerDay	||
		type ==	eLimitTypeNetLossPerMonth ||
		type ==	eLimitTypeSpendingLimitPerDay ||
		type ==	eLimitTypeSpendingLimitPerMonth ||
		type ==	eLimitTypeSportsNetLossPerDay ||
		type ==	eLimitTypeSportsNetLossPerMonth ||
		type ==	eLimitTypeSportsSpendingLimitPerDay ||
		type ==	eLimitTypeSportsSpendingLimitPerMonth;
...
void encodeZipPString(const PString& in, PString& out)
{
	// Compress the data
	PBlock block;
	PMemOutputStream stream(block);
	PDeflate deflator(stream);
	deflator.write((BYTE *)in.c_str(), in.length());
	deflator.close();

	// Encode the data in Base64
	CommSSLBase64::encode(out, block.ptr(), block.size());
}

void decodeUnzipPString(const PString& in, PString& out)
{
	out.assign("");
	// Decode the data from Base64
	PBlock compressed;
	CommSSLBase64::decode(compressed, in.c_str(), in.length());

	// Expand the data
	PMemInputStream stream(compressed);
	PInflate inflator(stream);
	for (;;)
	{
		BYTE buf[4096];
		size_t n = inflator.read(buf, sizeof (buf));
		if (n == 0)
			break;
