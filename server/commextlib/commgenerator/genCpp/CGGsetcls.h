...
#ifndef CGGsetcls_h_included
#define CGGsetcls_h_included

//******************************************************************************
// Set classes for text generators
//******************************************************************************
struct CGGsetcls : public CGGbasecls
{
	...
	virtual string getFieldJson(const CGfield* fld, const CGfieldtype* fieldType, const string& name, int margin, int counter)
	{
		...
		res.append(res1).append("{\n");
		res.append(res1).append("\t").append(fieldType->getZeroSubType()->getFieldTypeStr()).append(" refelem").append(to_string(counter + 1)).append(";\n");
		auto resInt = bcls->getFieldJson(fld, fieldType->getZeroSubType(), string("refelem") + to_string(counter + 1), margin + 1, counter + 1);
		res.append(resInt);
		res.append(res1).append("\t").append(name).append(".insert(move(refelem").append(to_string(counter + 1)).append("));\n");
		res.append(res1).append("}\n");
	...
	virtual vector<string> fillParse(const CGfield* fld, const CGfieldtype* fieldType, const string& name, const string& channel, CGmain* mainObj, bool isArray, bool& vecSmartCorr) override
	{
		...
		res1 = bcls->fillParse(fld, fieldType->getZeroSubType(), fld->getName()+"_data", channel, mainObj, isArray, vecSmartC);
		if (vecSmartC)
		{
		...
		for(auto elem : res1)
		{
			...
			res.push_back(string("\t\t").append(move(elem)));
			res.push_back(string("\t\t").append(name).append(".insert(").append("move(").append(fld->getName()).append("_data").append("));\n"));
		}
		res.push_back(string("\t}\n"));
		res.push_back(string("}\n"));
		return res;
	...
	virtual vector<string> fillCompose(const CGfield* fld, const CGfieldtype* fieldType, const string& name, const string& channel, CGmain* mainObj, bool isArray, bool& vecSmartCorr) override
	{
		...
		unique_ptr<CGGbasecls> bcls = CGGbasecls::getBaseCls(fieldType->getZeroSubType()->getFieldTypeStr());
		bool vecSmartC = false;
		res1 = bcls->fillCompose(fld, fieldType->getZeroSubType(), string("*(")+fld->getName()+"_iter)", channel, mainObj, isArray, vecSmartC);
		if (vecSmartC)
		{
	...
	vector<string> fillSelConvertCompose(const CGfield* fld, const CGfieldtype* fieldType, const string& name, CGmain* mainObj) override
	{
		if(fld->getSelConvertFuncName().length() && (!fld->isArray()))
		{
			...
			//res.push_back(string("\t\tdata.composeString(*(").append(fld->getName()).append("_iter));\n"));	

			res.push_back(string("\t\t").append(fieldType->getZeroSubType()->getFullTypeString()).append(" _temp_").append(fld->getName()).append(";\n"));
			res.push_back(string("\t\tbool _temp_bool_").append(fld->getName()).append(" = ").append(fld->getSelConvertFuncName()).append("(*(").append(fld->getName()).append("_iter), _temp_").append(fld->getName()).append(", this, \"").append(fld->getClassName()).append("\", \"").append(fld->getName()).append("\");\n"));
	...
	virtual vector<string> fillUpdateInternal(const CGfield* fld, const string& name, CGmain* mainObj)
	{
		...
		if(fld->getUpdateHandler2().size())
		{
			...
			res.push_back(string("\t\treturn false;\n"));
			res.push_back(string("\t}\n"));
			res.push_back(string("\t").append(name).append(".insert(move(val));\n"));
		...
		else
		{
			...
			res.push_back(string("\t").append(fld->getFullTypeString()).append("::iterator it = ").append(name).append(".find(val);\n"));
			res.push_back(string("\tif(it != ").append(name).append(".end())\n"));
			res.push_back(string("\t{\n"));
			res.push_back(string("\t\t").append(name).append(".erase(it);\n"));
			res.push_back(string("\t}\n"));
			res.push_back(string("\telse\n"));
			res.push_back(string("\t{\n"));
		...
		res.push_back(string("while(cnt--)\n"));
		res.push_back(string("{\n"));
		res.push_back(string("\tUINT32 val;\n"));
		res.push_back(string("\tparser.parseUINT32(val);\n"));
		res.push_back(string("}\n"));
		return res;
	}
	...
	vector<string> fillSelFields(const CGfield* fld, const CGfieldtype* fieldType, CGmain* mainObj) override
	{
		...
		else
		{
			...
			else
			{
				...
				if(fld->getSelConvertFuncName().length() && (!fld->isArray()))
				{
					res1.push_back(string("{\n"));
					res1.push_back(string("\t").append(fieldType->getZeroSubType()->getFullTypeString()).append(" _temp_").append(fld->getName()).append(";\n"));
					res1.push_back(string("\tbool _temp_bool_").append(fld->getName()).append(" = ").append(fld->getSelConvertFuncName()).append("(*(").append(fld->getName()).append("_iter), _temp_").append(fld->getName()).append(", this, \"").append(fld->getClassName()).append("\", \"").append(fld->getName()).append("\");\n"));
					res1.push_back(string("\tif(_temp_bool_").append(fld->getName()).append(")\n"));
					res1.push_back(string("\t\t").append("data.composeString(_temp_").append(fld->getName()).append(");\n"));
					res1.push_back(string("\telse\n"));
					res1.push_back(string("\t\tdata.composeString(*(").append(fld->getName()).append("_iter));\n"));	
					res1.push_back(string("}\n"));
				}
				else
				{
...
struct CGGpyrstringsetcls : public CGGbasecls
{
	virtual string fillFieldJson(const CGfieldtype* fieldType, const string& name1, const string& name2, int margin1, int margin2, int counter, bool last) override
	{
		string res, res1, res2;
	...
	vector<string> fillFieldAssignment(const CGfield* fld, const string& name1, const string& name2) override
	{
		...
		res.push_back(string("\tPStringSet::const_iterator ").append(name1).append("_iter;\n"));
		res.push_back(string("\tfor(").append(name1).append("_iter=").append(name2).append(".begin(); ")
			.append(name1).append("_iter!=").append(name2).append(".end(); ++")
			.append(name1).append("_iter)\n"));
		res.push_back(string("\t{\n"));

		res.push_back(string("\t\t").append(name1).append(".insert(*").append(name1).append("_iter);\n"));
	...
	virtual vector<string> fillFieldComparator(int counter, const CGfield* fld, const CGfieldtype* fieldType, const string& name1, const string& name2, const string& name3, bool negat, CGmain* mainObj) override
	{
		...
		res.push_back(string("\telse for(").append(iter1).append("=").append(name1).append(".begin(), ").append(iter2)
			.append("=").append(name2).append(".begin(); ").append(iter1).append("!=").append(name1).append(".end(); ++").append(iter1).append(", ++").append(iter2).append(")\n"));
		res.push_back(string("\t{\n"));
		res.push_back(string("\t\tif(strcmp(*").append(iter1).append(", *").append(iter2).append("))").append(name3).append("\n"));
		res.push_back(string("\t}\n"));
		res.push_back(string("}\n"));
		return res;
	}
	...
	virtual vector<string> fillFieldComparatorLog(int counter, const CGfield* fld, const CGfieldtype* fieldType, const string& name1, const string& name2, CGmain* mainObj) override
	{
		...
		res.push_back(string("\telse for(").append(iter1).append("=").append(name1).append(".begin(), ").append(iter2)
			.append("=").append(name2).append(".begin(); ").append(iter1).append("!=").append(name1).append(".end(); ++").append(iter1).append(", ++").append(iter2).append(")\n"));
		res.push_back(string("\t{\n"));
		res.push_back(string("\t\tif(strcmp(*").append(iter1).append(", *").append(iter2).append("))"));
		res.push_back(string("\t\t{\n"));
		res.push_back(string("\t\t\tPLog(\"Fields '").append(fld->getName()).append("' in class '").append(fld->getClassName()).append("' type '").append(fieldType->getFullTypeString()).append("' are NOT EQUAL by elements.\");\n"));
		res.push_back(string("\t\t\tresult = false;\n"));
		res.push_back(string("\t\t}\n"));
		res.push_back(string("\t}\n"));
		res.push_back(string("}\n"));
	...
	virtual vector<string> fillParse(const CGfield* fld, const CGfieldtype* fieldType, const string& name, const string& channel, CGmain* mainObj, bool isArray, bool& vecSmartCorr) override
	{
		...
		res.push_back(string("\tfor(size_t i=0; i<").append(fld->getName()).append("_count; ++i)\n"));
		res.push_back(string("\t{\n"));
		res.push_back(string("\t\tPString").append(" ").append(fld->getName()).append("_data;\n"));
		res.push_back(string("\t\tparser.parseStringP(").append(fld->getName()).append("_data);\n"));
		res.push_back(string("\t\t").append(name).append(".insert(").append("move(").append(fld->getName()).append("_data").append("));\n"));
		res.push_back(string("\t}\n"));
		res.push_back(string("}\n"));
		return res;
