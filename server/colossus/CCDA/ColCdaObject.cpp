...
/*static*/ CommServerPlainTextPasswordSidGuardFactory ColCdaServerObject::authGuardFactory;
/*static*/ CommClientNullGuardFactory ColCdaServerObject::clientNullGuardFactory;
...
bool ColCdaServerObject::registerAllFactories() // override
{
...
void ColCdaServerObject::startup()
{
	if (dumpFile.length())
	{
		try
...
/*virtual*/ void ColCdaServerObject::shutdown() // override
{
	if (dumpFile.length())
	{
		try
...
void ColCdaServerObject::setReply(CommMsgBody& reply, INT16 errCode, const char* errDesc/* = ""*/)
{
...
void ColCdaServerObject::postReply(GenericSrvConnection* conn, UINT32 msgId, CommMsgBody& body)
{
	if (conn)
	{
...
void ColCdaServerObject::processCommonServerMessage(UINT32 msgId, const CommMsgBody& body, ServerConnectionType connType, GenericSrvConnection* conn)
{
...
void ColCdaServerObject::processExampleSrvMessage(UINT32 msgId, const CommMsgBody & body, GenericSrvConnection* conn)
{
	CommMsgParser parser(body);
	switch (msgId)
	{
